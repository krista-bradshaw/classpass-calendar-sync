...
<d:propfind xmlns:d="DAV:">
  <d:prop><d:current-user-principal/></d:prop>
        ...
        resp = self._request('PROPFIND', f'{CALDAV_BASE}/.well-known/caldav', body, {'Depth': '0'})
...
<d:propfind xmlns:d="DAV:" xmlns:c="urn:ietf:params:xml:ns:caldav">
  <d:prop><c:calendar-home-set/></d:prop>
        ...
        home = re.search(r'calendar-home-set.*?<d:href>(.*?)</d:href>', resp.text, re.DOTALL)
        if not home:
...
<d:propfind xmlns:d="DAV:" xmlns:c="urn:ietf:params:xml:ns:caldav">
  <d:prop>
    ...
    <c:supported-calendar-component-set/>
        ...
        calendars = []
        for response in re.finditer(r'<d:response>(.*?)</d:response>', resp.text, re.DOTALL):
            ...
            if href:
                url  = href.group(1) if href.group(1).startswith('http') else CALDAV_BASE + href.group(1)
        ...
        if not match:
            raise Exception(f'Calendar "{CALENDAR_NAME}" not found. Available: {[c[0] for c in calendars]}')
  ...
  <c:filter>
    <c:comp-filter name="VCALENDAR">
      <c:comp-filter name="VEVENT">
        ...
        for response in re.finditer(r'<d:response>(.*?)</d:response>', resp.text, re.DOTALL):
            block = response.group(1)
            ical  = re.search(r'<c:calendar-data[^>]*>(.*?)</c:calendar-data>', block, re.DOTALL)
            href  = re.search(r'<d:href>(.*?)</d:href>', block)
            ...
            url     = href.group(1) if href.group(1).startswith('http') else CALDAV_BASE + href.group(1)
            events.append({'href': url, 'uid': uid, 'summary': summary, 'dtstart': dtstart})
    ...
    def create_event(self, booking):
        ...
        resp   = self._request('PUT', url, ical, {
...
def sync():
    ...
    for b in to_add:
        try:
            ...
            print(f'  ✓ Added: {b["className"]} @ {b["studio"]} on {b["startDate"][:10]}')
            created += 1
